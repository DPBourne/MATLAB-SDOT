function [g,Dg,H,actual_vols] = kantorovich2d(w,X,target_vols,bx,periodic,varargin)
...
%         X            - the positions of the seeds, an Nx2 array
%         target_vols  - target volumes, an Nx1 array, 
%                        which must satisfy sum(target_vols)=volume(bx)
%         bx           - box size, a 1x4 array, [xmin, ymin, xmax, ymax]
...
if(periodic)
    ...
    X=mod((X-[bx(1) bx(2)])*diag(1./[bx(3)-bx(1), bx(4)-bx(2)]),1)*diag([bx(3)-bx(1),bx(4)-bx(2)])+[bx(1) bx(2)];
...
%         X            - is the positions, an Nx2 array
%         target_vols  - target volumes, an Nx1 array
...
%% Catch errors and process arguments
    ...
    % Use mexPDall_2d to calculate the Laguerre diagram
    [actual_vols,transport_costs,~,vfn]=mexPDall_2d(bx,X,w,true);
    ...
    numNZ=sum(cellfun(@length,vfn(:,2)));
    ...
    for i=1:Nw
        ...
        % The face lengths and normals
        Edges_i=circshift(Vertices_Cell_i,-1)-Vertices_Cell_i;
        Edge_Lengths_i=vecnorm(Edges_i,2,2);
        
        % Outward normal to the edge
        Edge_Normals_i=(Edges_i*[0 -1;1 0])./Edge_Lengths_i;
        ...
        for j=1:Num_N_i
            ...
            % We only need to calculate if the neighbour is a not a part of the boundary (positive k indicates not part of boundary)
            if(k>0)
                % Length of the edge 
                a_ik=Edge_Lengths_i(j);
...
%% Catch errors
    ...
    total_vol=sum(target_vols);
    bx_vol=(bx(3)-bx(1))*(bx(4)-bx(2));

    bxv=[bx(3)-bx(1),bx(4)-bx(2)];
    ...
    poslength=@(x)sum(x>0);
    % Pre-allocate memory
    numNZ=sum(cellfun(poslength,vfn(:,2)))/2;
    ...
    for i=1:Nw
        ...
        % Edge midpoints
        Edge_Midpoints=0.5*(circshift(Vertices_Cell_i,-1)+Vertices_Cell_i);
        ...
        for j=1:Num_N_i
            ...
            if(k>i)  % Upper triangular elements only
                ...
                dist_ik=norm(xi-xk+del_k*diag(bxv));
...
% Turn off checking when used as an internal function
    ...
    % Use mexPDall_2d to calculate the Laguerre diagram
    [actual_vols,transport_costs,~,vfn]=mexPDall_2d(bx,X,w,false);
    ...
    for i=1:Nw
        ...
        % Outward normal to the edge
        Edge_Normals_i=Edges_i*[0 -1;1 0];
        ...
        for j=1:Num_N_i,
            ...
            % We only need to calculate if the neighbour is a not a part of the boundary (boundary is non-positive k)
            if(k>i)
